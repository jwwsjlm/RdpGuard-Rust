...
use rdpguard::{
    monitor::{AuthEvent, AuthResult, IpSummary, MonitorSnapshot, TcpConnection},
...
fn sample_snapshot() -> MonitorSnapshot {
    ...
    MonitorSnapshot {
        ...
        warnings: vec!["Security 日志读取失败".into()],
        auth_truncated: true,
...
fn keyboard_switches_pages_ranges_and_scrolls() {
    let mut app = MonitorApp::new(sample_snapshot());
    ...
    app.handle_key(KeyCode::Tab);
    assert_eq!(app.page(), MonitorPage::Connections);
...
fn all_three_pages_render_their_data_and_warnings() {
    ...
    app.handle_key(KeyCode::Tab);
    let connections = rendered_text(&app, 120, 24);
    assert!(connections.contains("当 前 连 接"));
    assert!(connections.contains("50000"));
}

#[test]
fn narrow_terminal_renders_without_panicking() {
    let app = MonitorApp::new(sample_snapshot());
